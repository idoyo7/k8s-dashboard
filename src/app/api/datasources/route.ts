...
import { saveConfig, getDatasources, getDatasourceById, getDatasourceAllowedNetworks } from '@/lib/app-config';
...
// No auth check — local homeserver deployment
function checkAdmin(_req: NextRequest): { isAdmin: boolean; error?: NextResponse } {
  return { isAdmin: true };
}
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ datasources, isAdmin: true });
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'generate-query') {
      const adminCheck = checkAdmin(request);
      if (adminCheck.error) return adminCheck.error;
      // AI query generation removed (Bedrock dependency removed)
      // TODO: Re-implement with OpenAI API for server-side query generation
      return NextResponse.json({ error: 'AI query generation not yet configured. Use direct queries.' }, { status: 501 });
    }
