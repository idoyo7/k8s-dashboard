...
// Handles AWS EKS ARNs (arn:aws:eks:...:cluster/name), slash-delimited paths,
// and plain context names.
function extractClusterName(ctx: string): string {
  if (!ctx) return '';
  // For any format ending with /something, return the last segment
  const m = ctx.match(/\/([^/]+)$/);
  return m ? m[1] : ctx;
}
