...
### Datasources (2)
- `datasources/page.tsx` — 외부 데이터소스 관리 (CRUD, 연결 테스트, 인증 설정, Allowed Networks)
- `datasources/explore/page.tsx` — 데이터소스 쿼리 탐색기 (직접 쿼리 실행, AI 쿼리 생성, 차트 시각화)
...
| `api/datasources/route.ts` | 외부 데이터소스 CRUD + 쿼리 실행 + AI 쿼리 생성 (SSRF 방지) |
...
### Datasources (2)
- `datasources/page.tsx` — External datasource management (CRUD, connection test, auth config, Allowed Networks)
- `datasources/explore/page.tsx` — Datasource query explorer (direct query, AI query generation, chart visualization)
...
| `api/datasources/route.ts` | External datasource CRUD + query execution + AI query generation (SSRF-protected) |
