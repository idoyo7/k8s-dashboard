export const queries = {
  ...
  // Running pods with node assignment (topology graph) / 실행 중인 파드 (토폴로지 그래프)
  eksNodes: `
  ...
  // K8s Services (topology graph) / K8s 서비스 (토폴로지 그래프)
  k8sServices: `
    SELECT
  ...
  // K8s Ingress (topology graph) / K8s 인그레스 (토폴로지 그래프)
  k8sIngress: `
    SELECT
      name, namespace,
      ingress_class_name,
